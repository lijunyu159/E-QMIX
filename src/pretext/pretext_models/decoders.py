...
class LSTM_DECODER(nn.Module):
    def __init__(self, obs_space_dict, config, task, latent_size):
        ...
        self.n_actions = 7
        self.state_action_size = 7 # 7个动作，用one-hot描述动作
        ...
        self.output_linear = nn.Sequential(
            ...
            nn.Linear(self.output_size // 2, self.state_action_size)
        )
    ...
    def _forward(self, z, rnn_hxs, seq_length, nenv):
        # z.shape is (B, T, N, F)
        z = z.permute(1, 0, 2, 3)  # seq_len, nenv, 2
        ...
        # else:
        # 仅在第一步使用初始化状态
        state = -2 * torch.ones(nenv, z.shape[2], self.state_action_size).cuda() # B, N, 8
        outputs = torch.zeros(seq_length, nenv, z.shape[2], self.state_action_size).cuda() # T, B, N, 8
        ...
        for i in range(seq_length):
            ...
            concat_state = torch.cat((state.reshape(nenv, -1, self.state_action_size), z[i]), dim=-1)  # nenv, 7
            # concat_state = z[i]
            concat_state = self.mlp_in(concat_state)  # nenv, 64
            # rnn input: [1, nenv, 64], hidden: [1, nenv, 256]
            state, hidden_states = self.RNN(concat_state.reshape(1, -1, concat_state.shape[-1]), hidden_states)
            # reshape state back to [nenv, 256] -> [nenv, 5]
            state = self.output_linear(state)
            outputs[i] = state.reshape(nenv, -1, state.shape[-1])

        return outputs, hidden_states

        # forward function for rl: returns critic output, actor features, and new rnn hidden state

    def forward(self, z, rnn_hxs, seq_len, infer=False):
        # if infer:
        #     # Test time
        #     seq_length = 1
        #     nenv = self.nenv  # 12
...
class MLP(nn.Module):
    ...
    def _forward(self, inputs, z, seq_length, nenv):
        robot_node = inputs.permute(1, 0, 2,3)  # seq_length, batch_size, 2
        # spatial_edges = inputs['pretext_spatial_edges'].permute(1, 0, 2)  # # seq_length, batch_size, 2
        z = z.permute(1, 0, 2,3)  # seq_len, batch_size, 1
        ...
        # concat_state = torch.cat((robot_node, z), dim=-1)
        concat_state = z
