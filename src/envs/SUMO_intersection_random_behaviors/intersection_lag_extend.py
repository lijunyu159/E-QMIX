...
from src.extract_conflict_2 import split_line_withgivepoint, gettwolineinterpoint

from src.extract_conflict_2 import cal_conflict
...
class MyIntersectionRandom(MultiAgentEnv):
    ...
    def __init__(self, **kwargs):
        self.cfg_dir = "/home/ljy/heuristic_based_qmix_republic/src/envs/SUMO_intersection_random_behaviors/main.sumocfg"
        ...
        # ====== 用于保存 SUMO-GUI 截图的设置（仅评估用） ======
        self.save_frames = kwargs.get("save_frames", False)
        self.frames_dir = kwargs.get("frames_dir", "./sumo_eval_frames")
        self.current_ep_dir = None    # 当前 episode 的帧目录
        self.view_id = None           # GUI 视图的 ID，用于 screenshot
        # ===============================================
        ...
        self.ControlledLanes, self.Linkinterdicts, self.turn2linksdicts, self.laneid2lengthdicts = self._init_conflict()

        self.Delta_t1_thesold, self.Delta_t2_thesold, self.L = 0.8, 1.2, 3.2

        # ======明确标注哪些车做的动作较差====== #
        self.env_infos = {"complete_flag": float(False), "collisions": 0.0, **self._empty_event_masks()}

    # ====== 明确标注哪些车做的动作较差 ====== #
    def _empty_event_masks(self):
        n = self.total_agents
    ...
    def step(self, actions):
        ...
        # ====== 新增：如果需要，保存当前这一步的 GUI 截图 ======
        if self.save_frames and self.render and self.current_ep_dir is not None:
            try:
                view_id = self.view_id
                if view_id is None:
                    # 理论上 reset 时已经设置过，这里再次兜底
                    view_ids = traci.gui.getIDList()
                    view_id = view_ids[0] if len(view_ids) > 0 else "View #0"

                frame_name = "frame_{:05d}.png".format(self.step_num)
                frame_path = os.path.join(self.current_ep_dir, frame_name)
        ...
        if self.shared_reward:
            rewards, slow_mask, danger_mask, alive_mask = self._get_shared_reward(next_observations, return_masks=True)

            # 写入 env_infos，runner 会通过 Pipe 收到（env_worker 已经在发 info 字段）:contentReference[oaicite:7]{index=7}
            self.env_infos["slow_mask"] = slow_mask
            self.env_infos["danger_mask"] = danger_mask
            self.env_infos["alive_mask"] = alive_mask
        else:
            rewards = self._get_rewards(next_observations)
    ...
    def get_state(self):
        # todo: 这里是不是可以描述一个包含所有HDVs的全局state，然后用RND
    ...
    def get_avail_actions(self):
        ...
        avail_actions = []
        fast_veh, slow_veh = self.safe_module()
        ...
        if len(self.CAVs_id_list) <= self.total_agents:
            for idx, id in enumerate(self.CAVs_id_list):
                ...
                avail_action = self.get_avail_agent_actions_our(fast_veh, slow_veh, agent_id=id)
    ...
    def get_avail_agent_actions_our(self, fast_veh, slow_veh, agent_id):

        # avail_actions = np.array([1 for _ in range(self.action_dim_per_veh)], dtype=np.int32) # 哪些动作可用，初始化时所有动作皆可用
        # avail_actions_tmp = np.array([0 for _ in range(self.action_dim_per_veh)]) # 屏蔽哪些动作
        ...
        if within_intersection:
            # print(self.fast_cav_ind)
            if int(agent_id) in fast_veh:
                avail_actions_tmp[: keep_speed_indx] = 1  # 快车更快
                avail_actions = avail_actions_tmp.copy()
            if int(agent_id) in slow_veh:
                avail_actions_tmp[keep_speed_indx+1: 2 * keep_speed_indx + 1] = 1  # 慢车更慢
                avail_actions = avail_actions_tmp.copy()
            return avail_actions
        return avail_actions
    ...
    def reset(self, env_args=None):
        ...
        if self.episode_num == 0:
            ...
            # ====== 新增：为当前 episode 准备截图目录，并确定 GUI 视图 ID ======
            if self.save_frames and self.render:
                # 根目录
                if not os.path.exists(self.frames_dir):
                    os.makedirs(self.frames_dir, exist_ok=True)

                # 每个 episode 一个子目录，episode_num 在 reset 中已经自增
                ep_name = "ep_{:03d}".format(self.episode_num)
                self.current_ep_dir = os.path.join(self.frames_dir, ep_name)
                if not os.path.exists(self.current_ep_dir):
                    os.makedirs(self.current_ep_dir, exist_ok=True)

                # 获取 GUI 视图 ID，用于后续 screenshot
                try:
                    # 通常只有一个视图，例如 "View #0"
                    view_ids = traci.gui.getIDList()
                    if len(view_ids) > 0:
                        self.view_id = view_ids[0]
                    else:
                        self.view_id = "View #0"
                except Exception:
                    # 兜底：如果拿不到列表，就假设是默认视图名
                    self.view_id = "View #0"
            # ===========================================================
        # return obs
    ...
    def _get_shared_reward(self, next_observations, return_masks=False):
        reward = 0
        # wt_list = []
        # ====== 记录不合理的车辆 ====== #
        slow_mask = np.zeros((self.total_agents,1),dtype=np.float32)
        danger_mask = np.zeros((self.total_agents, 1), dtype=np.float32)
        alive_mask = np.zeros((self.total_agents, 1), dtype=np.float32)

        wt = 0
        self.v_threshold = 0.5 / self.max_speed
        if_danger_dict = {t: False for t in self.CAVs_id_list}

        for vehID in self.CAVs_id_list:
            idx = int(vehID)  # 你的 CAV id 是 "0","1"... 这种形式

            if vehID not in self.curr_ID_list:
                continue
            alive_mask[idx, 0] = 1.0

            v_norm = self.obs_temp["speed"][vehID][-1] / self.max_speed
            if v_norm < self.v_threshold:
                reward -= 0.5
                slow_mask[idx, 0] = 1.0

            if_danger = self._within_danger_zone_at_intersection(vehID=vehID)
            if if_danger:
                reward -= 5
                danger_mask[idx, 0] = 1.0
        ...
        # ====== reward不重新计算 ====== #
        if done := self._get_done():
            pass
        if return_masks:
            return reward, slow_mask, danger_mask, alive_mask
        else:
            return reward
    ...
    def _init_conflict(self):
        ...
        sumocfgxmlfile = "/home/ljy/heuristic_based_qmix_republic_safe_module/src/envs/SUMO_intersection" \
                                 "_random_behaviors/main.sumocfg"
        traci.start(self.sumoCmd)
        interid = "J1"

        # 直接读取原始文件来获取，更简单一点
        # outrouxml='rou.rou.xml'
        path1 = os.path.dirname(os.path.realpath(__file__))
        xmlfile = "/home/ljy/heuristic_based_qmix_republic_safe_module/src/envs/SUMO_intersection" \
                          "_random_behaviors/" + "4_way_2_lanes.net.xml"
        # 解析 XML 文件
        tree = ET.parse(xmlfile)  # 用 xml.etree.ElementTree 解析这个 net.xml 文件，得到一个 tree 对象
        root = tree.getroot()  # 拿到 XML 的根节点（整个 SUMO 路网 XML 的根）
        linkslist = []  # 收集了所有 connection 里的 via 车道 ID，
        enterlaneid2linksdicts = {}
        turn2linksdicts = {}
        laneid2lengthdicts = {}
        for connection in root.findall('connection'):
            startedgeid = connection.get('from')
            if startedgeid[0] != ":":
                endedgeid = connection.get('to')
                fromLane = connection.get('fromLane')
                toLane = connection.get('toLane')
                onelink = connection.get('via')
                # interid=onelink[1: [ii for stri,ii in zip(onelink,range(0,len(onelink))) if ii=="_"][-2]]
                # print(interid)
                linkslist.append(onelink)
                laneid2lengthdicts[startedgeid + "_" + fromLane] = traci.lane.getLength(startedgeid + "_" + fromLane)
                try:
                    turn2linksdicts[(startedgeid, endedgeid)].append(onelink)
                except:
                    turn2linksdicts[(startedgeid, endedgeid)] = [onelink]
                enterlaneid = startedgeid + "_" + fromLane
                try:
                    enterlaneid2linksdicts[enterlaneid].append(onelink)
                except:

                    enterlaneid2linksdicts[enterlaneid] = [onelink]

        """
        预先把“所有连接车道两两之间的几何冲突点和到这个冲突点的距离算好，存进字典 Linkinterdicts，给后面 cal_conflict 用来算两车到冲突点的时间。
        """
        Linkinterdicts = {}
        for onelink in linkslist:

            lines1 = list(traci.lane.getShape(onelink))
            for otherlink in linkslist:
                if onelink != otherlink:
                    lines2 = list(traci.lane.getShape(otherlink))
                    interpoint = gettwolineinterpoint(lines1, lines2)
                    if interpoint is not None:
                        # print(lines1,lines2,interpoint)
                        splitline1, d1, splitline2, d2 = split_line_withgivepoint(lines1, interpoint)
                        splitline11, d11, splitline22, d22 = split_line_withgivepoint(lines2, interpoint)
                        Linkinterdicts[(onelink, otherlink)] = {"splitline1": splitline1, "d1": d1,
                                                                "splitline2": splitline2,
                                                                "d2": d2,
                                                                "splitline11": splitline11, "d11": d11,
                                                                "splitline22": splitline22, "d22": d22}

        # 进口道和出口道对应 turndir
        # L = 3.5  # 车道宽度
        # Delta_t1_thesold = 0.5  # 避免到达冲突点的时间差阈值
        # Delta_t2_thesold = 2  # 避免离开-到达冲突点的时间差阈值
        net = sumolib.net.readNet("/home/ljy/heuristic_based_qmix_republic_safe_module/src/envs/SUMO_intersection" \
                                  "_random_behaviors/" + "4_way_2_lanes.net.xml")
        node = net.getNode(interid)
        Controllededges = [e.getID() for e in node.getIncoming()]

        # Controllededges=traci.junction.getIncomingEdges(interid)
        Controllededges = [lane for lane in Controllededges if lane[0] != ":"]
        ControlledLanes = []
        for edgeid in Controllededges:
            for i in range(0, traci.edge.getLaneNumber(edgeid)):
                ControlledLanes.append(edgeid + "_" + str(i))
        self.ControlledLanes = ControlledLanes
        self.Linkinterdicts = Linkinterdicts
        self.turn2linksdicts = turn2linksdicts
        self.laneid2lengthdicts = laneid2lengthdicts
        return ControlledLanes, Linkinterdicts, turn2linksdicts, laneid2lengthdicts

    def safe_module(self):
        # 计算当前场景是否有碰撞风险，如果有碰撞风险，就不选择大加速度，否则正常选择动作
        # conflict_pair = [(), (), ...]元组里面有'typex.0'或'x.0'
        is_danger = 0
        conflict_pair = []
        fast_veh, slow_veh = cal_conflict(
            self.laneid2lengthdicts,
            self.turn2linksdicts,
            self.ControlledLanes, self.Delta_t1_thesold, self.Delta_t2_thesold, self.L, self.Linkinterdicts)
        # is_danger, conflict_pair = cal_conflict(
        #     self.laneid2lengthdicts,
        #     self.turn2linksdicts,
        #     self.ControlledLanes, self.Delta_t1_thesold, self.Delta_t2_thesold, self.L, self.Linkinterdicts)
        # cur_conflict_pair = conflict_pair[0]
        # conflict_type = conflict_pair[1]
        return fast_veh, slow_veh