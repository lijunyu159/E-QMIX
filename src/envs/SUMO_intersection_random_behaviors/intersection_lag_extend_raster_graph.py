...
class MyIntersectionRandom(MultiAgentEnv):
    ...
    def __init__(self, **kwargs):
        ...
        # ===================== Raster + Graph global state config ===================== #
        # This env originally returns a flattened concatenation of per-vehicle observations as "state".
        # For CNN/GNN style centralized state encoding, we additionally provide:
        #   - raster: BEV-like multi-channel grid centered at the junction
        #   - graph: fixed-size vehicle graph (nodes=total_agents) with a dense adjacency matrix
        #
        # Two return modes:
        #   - state_return_dict=True  -> get_state() returns a dict: {"raster","node_feat","adj","node_mask"}
        #   - state_return_dict=False -> get_state() returns a flattened (1, state_dim) numpy array
        #
        # NOTE: many MARL codebases assume get_state_size() is an int; we keep a flat dimension available.
        self.state_return_dict = kwargs.get("state_return_dict", False)

        # Raster parameters
        self.raster_size = int(kwargs.get("raster_size", 64))            # H=W
        self.raster_range_m = float(kwargs.get("raster_range_m", 60.0))  # cover [-range, +range] in both x/y
        self.raster_cell_m = (2.0 * self.raster_range_m) / self.raster_size
        # Channels: 0 junction_mask, 1 occ_all, 2 occ_cav, 3 speed_all, 4 in_junction_all
        self.raster_channels = int(kwargs.get("raster_channels", 5))

        # Graph parameters
        self.graph_max_nodes = int(kwargs.get("graph_max_nodes", self.total_agents))  # fixed node count
        self.graph_edge_dist_m = float(kwargs.get("graph_edge_dist_m", 35.0))         # connect if within this distance
        self.graph_dist_sigma = float(kwargs.get("graph_dist_sigma", 15.0))           # for exp(-d/sigma) edge weight
        # Node features:
        # [dx_norm, dy_norm, v_norm, sin_heading, cos_heading, in_junction, is_cav, enter_flag, leave_flag, intention]
        self.graph_node_feat_dim = int(kwargs.get("graph_node_feat_dim", 10))

        # Static cached layers for raster
        self.junction_center = None
        self._static_junction_mask = None  # (H,W) float32
        self._state_flat_dim = None
    ...
    def get_state(self):
        ...
        - If self.state_return_dict is True: returns a dict with raster+graph tensors:
              {"raster": (C,H,W), "node_feat": (N,F), "adj": (N,N), "node_mask": (N,)}
        - Else: returns a flattened numpy array with shape (1, state_dim), suitable for mixers like QMIX.
        """
        state_dict = self._get_raster_graph_state()
        if self.state_return_dict:
            return state_dict
        return self._pack_state_flat(state_dict)

    def get_state_size(self):
        """Returns the flattened state dimension (int)."""
        if self._state_flat_dim is None:
            self._state_flat_dim = self._calc_state_flat_dim()
        return int(self._state_flat_dim)
    ...
    def reset(self, env_args=None):
        ...
        if self.episode_num == 0:
            ...
            self.junction_center = np.array(traci.junction.getPosition(self.junction_name), dtype=np.float32)
            self._maybe_init_static_raster_layers()
